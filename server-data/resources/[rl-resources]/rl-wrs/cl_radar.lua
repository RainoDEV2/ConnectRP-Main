    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        local vehicle = GetVehiclePedIsIn( ped, false )
            ...
            if ( radarEnabled ) then 
                RLCore.Functions.Notify( "Radar enabled." )
            else 
                RLCore.Functions.Notify( "Radar disabled.",'error' )
            end 
...
AddEventHandler( 'wk:toggleRadar', function()
    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        RLCore.Functions.Notify( "Not in vehicle.",'error' )
    end 
end )
...
function ToggleSpeedType()
    ...
    RLCore.Functions.Notify( "Speed type MPH." )
end 

function ToggleLockBeep()
    if ( radarInfo.lockBeep ) then 
        radarInfo.lockBeep = false 
        RLCore.Functions.Notify( "Radar beep disabled.",'error' )
    else 
    ...
    while true do 
        ...
        if GetPedInVehicleSeat( veh, -1 ) == ped then
            ...
            if ( IsDisabledControlPressed( 1, 36 ) and IsDisabledControlJustPressed( 1, 243 ) and IsPedSittingInAnyVehicle( ped ) and currentJob == 'police' ) then 
                ...
                if lockplates then
                    RLCore.Functions.Notify('Radar Locked', 'error')
                else
                    RLCore.Functions.Notify('Radar Unlocked')
                end
            end 
            ...
            if ( not IsDisabledControlPressed( 1, 36 ) and IsDisabledControlJustPressed( 1, 243 ) and IsPedSittingInAnyVehicle( ped ) and currentJob == 'police' ) then 
                ResetFrontFast()
                ResetRearFast()
                RLCore.Functions.Notify('Lockfast Reset')
            end 
