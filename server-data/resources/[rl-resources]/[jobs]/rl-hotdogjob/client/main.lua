...
RegisterNetEvent('rl-hotdogjob:client:updateRunningStand')
AddEventHandler('rl-hotdogjob:client:updateRunningStand', function(shouldRemove, serverID)
        ...
        while true do
            ...
            if StandObject ~= nil then
                if ClosestObject ~= nil and ClosestObject == StandObject then
                    ...
                    if ObjectDistance < 1.0 then
                        DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Prepare Hotdogs')
                        if IsControlJustPressed(0, Keys["E"]) then
                            StartHotdogMinigame()
                        end
                    end
...
RegisterNetEvent('rl-hotdogjob:client:UpdateReputation')
AddEventHandler('rl-hotdogjob:client:UpdateReputation', function(JobRep)
    PlayerData.metadata["jobrep"] = JobRep
    UpdateLevel()
end)

RegisterNetEvent('rl-hotdogjob:client:ToggleSell')
AddEventHandler('rl-hotdogjob:client:ToggleSell', function(data)
...
function ToggleSell()
    ...
    if StandObject ~= nil then
        if dist < 5.0 then
            ...
            RLCore.Functions.Notify('You are too far from your Hotdog Stand..', 'error')
        end
    else
        RLCore.Functions.Notify('You dont have a Hotdog stand', 'error')
    end
end
...
function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
        ...
        if PlayerDist > 15.0 then
            ...
            RLCore.Functions.Notify('Youre too far from your stall...', 'error')
            break
        end
        Citizen.Wait(100)
    end
    ...
    while pedDist < OffsetData.Distance and SellingData.HasTarget do
        ...
        if PlayerDist < 7.5 then
            if SellingData.Hotdog ~= nil then
                ...
                RLCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '[7] Sale '..HotdogsForSale..'x for $'..(HotdogsForSale * SellingPrice)..' / [8] Reject')
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    RLCore.Functions.Notify('Customer refused!', 'error')
                    SellingData.HasTarget = false
                ...
                SellingData.Hotdog = GetAvailableHotdog()
                RLCore.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, 'You have no stock ../ [8] Refuse customer')
...
function StartHotdogMinigame()
    PrepareAnim()
    TriggerEvent('rl-keyminigame:show')
    TriggerEvent('rl-keyminigame:start', FinishMinigame)
end
    ...
    if DoesEntityExist(StandObject) then
        RLCore.Functions.TriggerCallback('rl-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                ...
                RLCore.Functions.Notify('You have received your deposit back!', 'success')
            else
                RLCore.Functions.Notify('Failed', 'error')
            end
        end)
    else
        RLCore.Functions.Notify('Your hot dog stall was nowhere to be seen, you did NOT receive your deposit back!', 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsSelling = false
...
AddEventHandler('rl-hotdogjob:staff:DeletStand', function()
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Citizen.Wait(100)            
            DeleteEntity(Object)
            RLCore.Functions.Notify('Hot dog stand removed!')
        end
    end
end)
