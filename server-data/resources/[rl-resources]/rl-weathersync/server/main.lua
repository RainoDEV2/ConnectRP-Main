...
RegisterServerEvent('rl-weathersync:server:RequestStateSync')
AddEventHandler('rl-weathersync:server:RequestStateSync', function()
...
RegisterServerEvent('rl-weathersync:server:setWeather')
...
RegisterServerEvent('rl-weathersync:server:toggleBlackout')
AddEventHandler('rl-weathersync:server:toggleBlackout', function()
    ToggleBlackout()
end)

RegisterServerEvent('rl-weathersync:server:setTime')
AddEventHandler('rl-weathersync:server:setTime', function(hour, minute)
...
function NextWeatherStage()
    ...
    elseif CurrentWeather == "CLEARING" or CurrentWeather == "OVERCAST" then
        ...
        if new == 1 then
            if CurrentWeather == "CLEARING" then CurrentWeather = "RAIN" end
        elseif new == 2 then
    ...
    elseif CurrentWeather == "RAIN" then
        CurrentWeather = "CLEARING"
    elseif CurrentWeather == "XMAS" then
        CurrentWeather = "CLEAR"
    end
    TriggerEvent("rl-weathersync:server:RequestStateSync")
end
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1000)
        local newBaseTime = os.time(os.date("!*t"))/2 + 180
        if freezeTime then
...
--[[RLCore.Commands.Add("snowball", "Pick up snowball", {}, false, function(source, args)
    if CurrentWeather == "XMAS" then
        TriggerClientEvent("inventory:client:PickupSnowballs", source)
    else
        TriggerClientEvent('RLCore:Notify', source, "Not the right time of year :(", "error")
    end
end)]]--