...
RLCore.Commands.Add("mdt", "Toggle police mdt", {}, false, function(source, args)
	local src = source
	local Player = RLCore.Functions.GetPlayer(source)
...
AddEventHandler("mdt:performOffenderSearch", function(query)
	...
	RLCore.Functions.ExecuteSql(false, 'SELECT * FROM `players` WHERE `citizenid` = "'..query..'" OR `charinfo` LIKE "%'..query..'%"', function(result)
