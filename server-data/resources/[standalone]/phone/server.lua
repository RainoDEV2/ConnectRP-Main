...
AddEventHandler('Tweet', function(handle, data, time)
    ...
    print(json.encode(handle))
    print(json.encode(data))
    print(json.encode(time))
...
AddEventHandler('deleteContact', function(name, number)
    local xPlayer = RLCore.Functions.GetPlayer(source)
    local src = source
    exports.ghmattimysql:execute('DELETE FROM phone_contacts WHERE name = @name AND number = @number LIMIT 1', {
        ['name'] = name,
        ['number'] = number
    }, function (result)
        TriggerClientEvent('refreshContacts', src)
...
function getIdentifierByPhoneNumber(phone_number) 
    local result = exports.ghmattimysql:execute("SELECT '" .. xPlayer.PlayerData.citizenid .. "' FROM players WHERE players.phone_number = @phone_number", {
...
AddEventHandler('phone:callContact', function(targetnumber, toggle)
    ...
    local srcIdentifier = RLCore.Functions.GetIdentifier()
...
AddEventHandler('phone:getSMS', function()
    ...
    local result = exports.ghmattimysql:execute("SELECT * FROM phone_messages WHERE receiver = @mynumber OR sender = @mynumber ORDER BY id DESC", {['@mynumber'] = mynumber})
...
AddEventHandler('phone:sendSMS', function(receiver, message)
    ...
    exports.ghmattimysql:execute('INSERT INTO phone_messages (sender, receiver, message) VALUES (@sender, @receiver, @message)', {
        ['sender'] = mynumber,
        ['receiver'] = receiver,
        ['message'] = message
    }, function(result)
...
AddEventHandler('phone:serverGetMessagesBetweenParties', function(sender, receiver, displayName)
    ...
    local xPlayer = RLCore.Functions.GetPlayer(src)
    local mynumber = getNumberPhone(xPlayer.identifier)
    local result = exports.ghmattimysql:execute("SELECT * FROM phone_messages WHERE (sender = @sender AND receiver = @receiver) OR (sender = @receiver AND receiver = @sender) ORDER BY id ASC", {['sender'] = sender, ['receiver'] = receiver})
...
RegisterCommand("ph", function(source, args, rawCommand)
    local src = source
    local xPlayer = RLCore.Functions.GetPlayer(src)
    local identifier = RLCore.Functions.GetIdentifier()
...
function getIdentity(target)
	--local identifier = GetPlayerIdentifiers(target)[1]
	--local result = exports.ghmattimysql:execute("SELECT * FROM players WHERE identifier = @identifier", {['@identifier'] = identifier})
	--if result[1] ~= nil then
	--	local identity = result[1]
...
RegisterCommand("payphone", function(source, args, raw)
    ...
    if xPlayer.get('money') >= 25 then
        ...
        TriggerClientEvent("banking:removeBalance", source, 25)
    else
