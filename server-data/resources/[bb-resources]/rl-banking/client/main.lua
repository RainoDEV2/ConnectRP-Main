...
AddEventHandler('RLCore:Client:OnPlayerLoaded', function(data)
    RLCore.Functions.TriggerCallback('rl-banking:server:requestBanks', function(banksdata)
        banks = banksdata
        playerData = data
        playerLoaded = true
        createBlips()
...
function openAccountScreen()
    RLCore.Functions.TriggerCallback('rl-banking:getBankingInformation', function(banking)
        if banking ~= nil then
            InBank = true
...
function atmRefresh()
    RLCore.Functions.TriggerCallback('rl-banking:getBankingInformation', function(infor)
        InBank = true
        SetNuiFocus(true, true)
        SendNUIMessage({
            status = "refreshatm",
    ...
    for k, v in pairs(banks) do 
        if v.bankType == "Paleto" and v.moneyBags ~= nil and v.bankOpen then
            ...
            if moneyBagDist < 1.0 then
                RLCore.Functions.Progressbar("accessing_atm", "Cashier Counting Bag..", 60000, false, true, {
