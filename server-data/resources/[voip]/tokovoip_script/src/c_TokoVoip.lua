	...
	Citizen.CreateThread(function()
		while (true) do
			...
			elseif (IsControlJustPressed(0, self.keyProximity)) then -- Switch proximity modes (normal / whisper / shout)
				...
				TriggerEvent('mumble:updateMumbleInfo', self.mode)
			...
			if (IsControlPressed(0, self.radioKey) and self.plugin_data.radioChannel ~= -1 and self.config.radioEnabled) then -- Talk on radio
				...
				if (lastTalkState == false and self.myChannels[self.plugin_data.radioChannel] and self.config.radioAnim) then
					if (not string.match(self.myChannels[self.plugin_data.radioChannel].name, "Call") and not IsPedSittingInAnyVehicle(ped)) then
	...
	while true do
		...
		for _, v in pairs(TokoVoipConfig.wisperZones) do
			if #(GetEntityCoords(PlayerPedId()) - v) < 20.0 then
				...
				TriggerEvent('mumble:updateMumbleInfo', 1)
			end
		end
