TokoVoipConfig = {
	...
	keySwitchChannels = 83, -- Keybind used to switch the radio channels
	...
	keyProximity = 217, -- Keybind used to switch the proximity mode
	...
	wsServer = "176.31.248.216:3000", -- Address of the websocket server
	...
	plugin_data = {
		...
		TSChannel = "TokoVOIP Room",
		TSPassword = "999", -- TeamSpeak channel password (can be empty)
		...
		TSChannelWait = "TokoVOIP AFK", -- You NEED tokovoip in the wait channel name!

		-- Blocking screen informations
		TSServer = "188.40.166.148:9997", -- TeamSpeak server address to be displayed on blocking screen
		TSChannelSupport = "AFK", -- TeamSpeak support channel name displayed on blocking screen
	...
	if (resource == GetCurrentResourceName()) then	--	Initialize the script when this resource is started
		Citizen.CreateThread(function()
			if(TokoVoipConfig.plugin_data.localName == '') then
				TokoVoipConfig.plugin_data.localName = "ply-" .. GetPlayerServerId(PlayerId()) -- Set the local name
			end
		end);
