...
-- NET EVENTS
-------------------------------------------------------------------------------
RegisterNetEvent("tc-slots:enterBets")
AddEventHandler("tc-slots:enterBets", function ()
    local bets = KeyboardInput("Enter bet value:", "", Config.MaxBetNumbers)
    if tonumber(bets) ~= nil then
    	TriggerServerEvent('tc-slots:BetsAndMoney', tonumber(bets))
    else
		RLCore.Functions.Notify('You need to enter numbers (9999 is max bet).')
    end
end)

RegisterNetEvent("tc-slots:UpdateSlots")
AddEventHandler("tc-slots:UpdateSlots", function(lei)
...
RegisterNUICallback('exitWith', function(data, cb)
	...
	open = false
	TriggerServerEvent("tc-slots:PayOutRewards", data.coinAmount)
end)
	...
	while true do
		...
		for i=1, #Config.Slots, 1 do
			if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.Slots[i].x, Config.Slots[i].y, Config.Slots[i].z, true) < 2  then
				...
				wTime = 0
				langaAparat = true
				DisplayHelpText("Press ~INPUT_PICKUP~ to test your luck at slot machine")
...
Citizen.CreateThread(function ()
	while true do
		...
		elseif IsControlJustReleased(0, 38) and langaAparat then
			TriggerEvent('tc-slots:enterBets')
		end
	end
end)
