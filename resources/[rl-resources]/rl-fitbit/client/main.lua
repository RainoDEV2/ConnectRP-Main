...
RegisterNetEvent('rl-fitbit:use')
AddEventHandler('rl-fitbit:use', function()
  openWatch(true)
end)
...
RegisterNUICallback('setFoodWarning', function(data)
    ...
    TriggerServerEvent('rl-fitbit:server:setValue', 'food', foodValue)

    RLCore.Functions.Notify('Fitbit: Food warning set to '..foodValue..'%')
end)

RegisterNUICallback('setThirstWarning', function(data)
    local thirstValue = tonumber(data.value)

    TriggerServerEvent('rl-fitbit:server:setValue', 'thirst', thirstValue)

    RLCore.Functions.Notify('Fitbit: Thirst warning set to '..thirstValue..'%')
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            RLCore.Functions.TriggerCallback('rl-fitbit:server:HasFitbit', function(hasItem)
                if hasItem then
